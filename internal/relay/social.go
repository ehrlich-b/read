...
// ListPostsByDomain returns visible posts whose link matches the given domain.
func (s *RelayStore) ListPostsByDomain(domain, sort string, limit int) ([]*SocialEmbedding, error) {
	// Escape LIKE wildcards in domain to prevent unintended matching
	domain = strings.ReplaceAll(domain, "%", "")
	domain = strings.ReplaceAll(domain, "_", "\\_")
	domainFilter := `AND (link LIKE '%://' || ? || '/%' ESCAPE '\' OR link LIKE '%://www.' || ? || '/%' ESCAPE '\')`
	...
	rows, err := s.db.Query(
		`SELECT id, user_id, link, text, title, centerpoint, slug, embedding, embedding_512, centroid_512, effective_512, kind, visible, mass, upvotes_24h, decayed_mass, swallowed, created_at, updated_at, published_at
		 FROM social_embeddings WHERE kind = 'post' AND visible = 1 `+domainFilter+` `+where+`
		 ORDER BY `+orderBy+` LIMIT ?`, domain, domain, limit)
	if err != nil {
		return nil, fmt.Errorf("list posts by domain: %w", err)
	}
	defer rows.Close()
...
// AnchorSlugsByWeight returns anchor slugs sorted by total mass of posts in the last 30 days (descending).
// Anchors with no recent posts are included at the end, alphabetically.
func (s *RelayStore) AnchorSlugsByWeight() ([]string, error) {
	rows, err := s.db.Query(
		`SELECT a.slug, COALESCE(SUM(p.mass), 0) as weight
		 FROM social_embeddings a
		 LEFT JOIN post_anchors pa ON pa.anchor_id = a.id
		 LEFT JOIN social_embeddings p ON p.id = pa.post_id
		   AND p.visible = 1 AND p.kind = 'post'
		   AND COALESCE(p.published_at, p.created_at) > datetime('now', '-30 days')
		 WHERE a.kind = 'anchor' AND a.visible = 1 AND a.slug IS NOT NULL
		 GROUP BY a.slug
		 ORDER BY weight DESC, a.slug ASC`)
	if err != nil {
		return nil, fmt.Errorf("anchor slugs by weight: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		var slug string
		var weight float64
		if err := rows.Scan(&slug, &weight); err != nil {
			return nil, fmt.Errorf("scan anchor slug weight: %w", err)
		}
		out = append(out, slug)
	}
	return out, rows.Err()
